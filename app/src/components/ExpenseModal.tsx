...
export function ExpenseModal(props: Props) {
  ...
  const [requestedUserId, setRequestedUserId] = useState<string | null>(expense?.requested_user_id ?? null)
  ...
  return (
    ...
    >
      ...
      <div
        ref={modalRef}
        className="relative w-full max-w-lg bg-surface rounded-2xl shadow-xl border border-border-app overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-border-app">
          ...
          <button onClick={props.onClose} className="p-1 rounded-lg text-text-muted hover:bg-surface2 hover:text-text-primary transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-5 py-4 space-y-4 max-h-[65vh] overflow-y-auto">
          {/* Título */}
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1">{t('expenses.form.titleLabel')}</label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder={t('expenses.form.titlePlaceholder')}
              className="w-full px-3 py-2 rounded-lg bg-surface2 border border-border-app text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-brand"
              autoFocus
            />
          </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                ...
                className={`px-2 py-0.5 rounded text-xs font-medium transition-colors ${
                  !labelId ? 'bg-brand/15 text-brand' : 'text-text-muted hover:bg-surface2'
                }`}
              >
                {t('common.none')}
              </button>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 rounded-lg bg-surface2 border border-border-app text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-brand resize-none"
            />
          </div>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                    paymentMethod === m
                      ...
                      : 'text-text-muted hover:bg-surface2 hover:text-text-secondary border border-border-app'
                  }`}
                >
                  {t(`expenses.${m}`)}
                </button>
          ...
          {requestedUserId && (
            <div>
              ...
              {splitType === 'custom' && (
                <div className="mt-2">
                  <label className="block text-xs font-medium text-text-secondary mb-1">{t('expenses.splitAmount')}</label>
                  <input
                    type="text"
                    inputMode="decimal"
          ...
          {isEdit && isRequested && expense && (
            <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-800">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
                {expense.created_by_username} {t('expenses.requestPayment')}: {fmtEurForDisplay(expense)}
              </p>
          ...
          {/* They paid badge */}
          {isEdit && expense && expense.requested_user_id && expense.paid_by_requested && !isRequested && (
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border-app">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  deleteArmed
                    ...
                    : 'text-text-muted hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-900/20'
                }`}
              >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-1.5 rounded-lg text-sm text-text-muted hover:bg-surface2 hover:text-text-primary transition-colors"
            >
              {t('common.cancel')}
            </button>
            <button
            ...
            >
              {saving ? t('expenses.form.creating') : isEdit ? t('common.save') : t('expenses.form.create')}
            </button>
          </div>
        </div>
      </div>
...
function fmtEurForDisplay(expense: Expense): string {
  let amount = expense.amount_cents
  if (expense.split_type === 'half') amount = Math.round(amount / 2)
  if (expense.split_type === 'custom' && expense.split_amount_cents) amount = expense.split_amount_cents
